...
CTX_COLOR_PCT="$CTX_USED"
if [ "${CTX_TOTAL:-0}" -ge 900000 ]; then
  CTX_COLOR_PCT=$(( CTX_USED * CTX_TOTAL / 800000 ))
  [ "$CTX_COLOR_PCT" -gt 100 ] && CTX_COLOR_PCT=100
fi

CTX_INFO="$(color_for_pct "$CTX_COLOR_PCT")${CTX_USED}%${RESET}"

USAGE_LIMITS=""
USAGE_CACHE="$CONFIG_DIR/claude_usage_cache"
# CACHE_MTIME=$(stat -f %m "$USAGE_CACHE" 2>/dev/null || echo 0)
# if [ $(( $(date +%s) - CACHE_MTIME )) -gt 180 ]; then
...
if [ -f "$USAGE_CACHE" ]; then
  ...
  EXTRA_USED=$(printf '%s' "$USAGE_JSON" | jq -r '.extra_usage.used_credits // empty')
  if [ -n "$EXTRA_USED" ]; then
    EXTRA_USED_INT=${EXTRA_USED%.*}
    if [ "$EXTRA_USED_INT" -gt 0 ]; then
      EXTRA_TEXT="${RED}\$${EXTRA_USED}${RESET}"
      [ -n "$USAGE_LIMITS" ] && USAGE_LIMITS="$USAGE_LIMITS $EXTRA_TEXT" || USAGE_LIMITS="$EXTRA_TEXT"
    fi
  fi
fi

if [ -s "${USAGE_CACHE}.err" ]; then
  CURL_ERR_MARK="${RED}⚠${RESET}"
  [ -n "$USAGE_LIMITS" ] && USAGE_LIMITS="$USAGE_LIMITS $CURL_ERR_MARK" || USAGE_LIMITS="$CURL_ERR_MARK"
fi
...
echo "${CONF:+$CONF:}$MODEL $CTX_INFO | $CWD_DISPLAY${GIT_INFO:+ | $GIT_INFO}${USAGE_LIMITS:+ | Usage: $USAGE_LIMITS}"
