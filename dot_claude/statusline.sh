...
color_for_pct() {
  ...
  elif [ "$pct" -ge 65 ]; then printf '%s' "$YELLOW"
...
CTX_INFO="$(color_for_pct "$CTX_USED")${CTX_USED}%${RESET} $(format_size "$CTX_CONSUMED")/$(format_size "$CTX_SIZE")"

USAGE_LIMITS=""
USAGE_CACHE="/tmp/claude/claude_usage_cache"
...
if [ $(( $(date +%s) - CACHE_MTIME )) -gt 600 ]; then
  ...
  if [ -n "$TOKEN" ]; then
    USAGE_JSON=$(curl -s --proxy http://127.0.0.1:12334 \
