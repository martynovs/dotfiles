...
class BrewPackage implements Package {
    name: string;
    brew_tap?: string;
    from_source_on_arm_linux?: boolean;
    ...
    sans_arm(): Package {
        this.from_source_on_arm_linux = true;
        return this;
    }
...
export function cargo_bin(name: string): CargoPackage { return new CargoPackage(name, true); }
...
class PackageSet {
    ...
    addBrewFromSource(name: string, brew_tap: string | undefined) {
        this.brew_packages_from_source.push(name);
        if (brew_tap) this.brew_taps.push(brew_tap);
    }
...
class BasicInstaller {
    ...
    printPackageSetScript(pkgSet: PackageSet): void {
        ...
        for (const bin of pkgSet.cargo_binaries) {
            print_cmd(`cargo binstall -y ${bin} || echo 'Failed cargo binstall ${bin}'`);
        }
        for (const pkg of pkgSet.cargo_packages) {
            print_cmd(`cargo install --locked ${pkg} || echo 'Failed cargo install ${pkg}'`);
        }
...
class LinuxInstaller extends BasicInstaller {
    ...
    brewPackage(pkg: BrewPackage): void {
        if (is_arm && pkg.from_source_on_arm_linux) { this.packages.addBrewFromSource(pkg.name, pkg.brew_tap); }
