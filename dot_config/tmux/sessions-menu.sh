...
EOF
    ...
    # -M: enable mouse handling (run-shell menus carry no mouse event, so tmux
    #     would otherwise set MENU_NOMOUSE and close the menu on button release)
    # -O: don't close when the button is released outside the menu
    set -- "$@" -M -O -T "#[align=centre]$title" -x C -y C
