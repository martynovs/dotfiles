...
function _git_main_dir -d 'print path of the main worktree'
    set -l main (git worktree list --porcelain | head -1 | string replace 'worktree ' '')
    test -n "$main"; or return 1
    echo $main
end

function _cd_git_main -d 'cd to the main worktree'
    set -l main (_git_main_dir)
    test -n "$main"; or return 1
    cd $main
end
...
function we -d 'switch worktree (interactive picker with no args)'
    set -l win_id (tmux display-message -p '#{window_id}')
    _cd_git_main; or return 1
...
function wr -d 'switch worktree to MR by number or search text'
    ...
    or return 1
    # switch from the main worktree: a worktree's symlinked node_modules etc. break it
    _cd_git_main; or return 1
    wt switch mr:$mr_id; or return 1
    _mr_rename_tmux_window $mr_id $win_id
end

function wec -d 'switch worktree (create branch from current if missing) and open claude'
    if test -z "$argv[1]"
        we; or return 1
    else if git show-ref --quiet --verify "refs/heads/$argv[1]"
        we $argv; or return 1
    else
        # pin the base to this worktree's HEAD: `@` would re-resolve once we cd to main
        set -l base (git rev-parse HEAD); or return 1
        _cd_git_main; or return 1
        wt switch --create $argv[1] --base $base; or return 1
    end
